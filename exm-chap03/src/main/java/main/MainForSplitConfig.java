...
public class MainForSplitConfig {

	private static ApplicationContext ctx = null;

	public static void main(String[] args) throws IOException {
		ctx = new GenericXmlApplicationContext("classpath:appCtxSplit1.xml", "classpath:appCtxSplit2.xml");
		...
		while (true) {
			...
			if (command.startsWith("new ")) {
				...
				System.out.println("종료합니다. (with split config)");
				break;
			} else {
				printHelp();
			}
	...
	private static void ProcessChangeCommand(String[] args) {
		...
		ChangePasswordService chgSvc = ctx.getBean("chgSvc", ChangePasswordService.class);
	...
	private static void ProcessPrintMember(String[] args) {
		MemberListPrinter printSvc = ctx.getBean("printSvc", MemberListPrinter.class);
	...
	private static void ProcessPrintVersion(String[] args) {
		VersionPrinter printer = ctx.getBean("versionPrint", VersionPrinter.class);
		printer.print();
	}
}
